...
function startDetection() {
    faceDetection = setInterval(async () => {
        ...
        const eyesClosed = detections.some((face) => face.expressions["neutral"] > 0.8);
        const now = new Date().getTime();

        if (eyesClosed) {
            if (now - lastEyeClosedTime > alertThreshold) {
                // Speak alert
                const alertUtterance = new SpeechSynthesisUtterance("Alert: Warning, eyes are closed!");
                speechSynthesis.speak(alertUtterance);
                // Update last eye closed time
                lastEyeClosedTime = now;
            }
        } else {
            if (lastEyeClosedTime !== 0) {
                // Stop alert if eyes are open
